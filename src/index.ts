...
const RATE_LIMIT    = parsePositiveInt(process.env.RATE_LIMIT_PER_MINUTE, 60,    "RATE_LIMIT_PER_MINUTE");
const QUERY_TIMEOUT = parsePositiveInt(process.env.QUERY_TIMEOUT_MS,      30000, "QUERY_TIMEOUT_MS");
const MAX_ROWS      = parsePositiveInt(process.env.MAX_ROWS,               1000,  "MAX_ROWS");
...
function enforceLimit(sql: string): { sql: string; injected: boolean } {
  if (LIMIT_PATTERN.test(sql)) return { sql, injected: false };
  return { sql: `${sql.trim()} LIMIT ${MAX_ROWS}`, injected: true };
}
...
// Pre-compiled patterns: direct start or after a CTE (WITH ... keyword ...)
const WRITE_PATTERNS = WRITE_KEYWORDS.map((kw) => ({
  direct: new RegExp(`^${kw}\\s`),
  cte: new RegExp(`^WITH[\\s\\S]*?\\s${kw}\\s`),
}));
...
function log(parts: Record<string, string | number | undefined>): void {
...
async function main() {
  ...
  console.error(`PostgreSQL Read-Only MCP Server v2.0.0 started`);
  console.error(`Environments: ${ENV_NAMES.join(", ")}`);
  console.error(`Mode: READ-ONLY (writes blocked at application level)`);
  console.error(`Protections: rate=${RATE_LIMIT}/min | timeout=${QUERY_TIMEOUT}ms | max_rows=${MAX_ROWS}`);
}
